...
class DialerReports extends Component {
    ...
    save() {
        ...
        console.log(data);
        this.props.createReport(data, function () {
            self.cancel();
        });
    }
        ...
        return (
            <CardBody>
                <Row>
                    <Col>
                        <FormGroup>
                            ...
                            <Input type="text" id="name" placeholder="Enter the report name" required value={this.state.report_name} onChange={(e) => this.setState({ report_name: e.target.value })} />
                            { /*<div className="mt-1">
                                {
                                    ["PDF", "CSV", "HTML"].map(type => {
                                        return <FormGroup key={type} check inline>
                                            <Label check><Input type="radio" name="reportType" value={type} checked={this.state.report_type === type} onChange={e => this.setState({ report_type: e.currentTarget.value })} /> {type}</Label>
                                        </FormGroup>
                                    })
                                }
                            </div>*/}
                ...
                <Row>
                    {/* <Col>
                        <FormGroup>
                            <Label htmlFor="ccnumber">Matured Days</Label>
                            <Input type="text" id="matured-days" placeholder="Matured Days" required value={this.state.mature_days} onChange={(e) => this.setState({ mature_days: e.target.value })} />
                        </FormGroup>
                    </Col> */}
                    <Col>
                        <FormGroup>
                ...
                <Row>
                    ...
                    {/* <Col>
                        <FormGroup>
                            <Label htmlFor="ccnumber">Campaign Created Date</Label>
                            <DateRangePicker
                                startDate={this.state.camapign_created_on_start}
                                startDateId="cc_startDate"
                                endDate={this.state.camapign_created_on_end}
                                endDateId="cc_endDate"
                                onDatesChange={({ startDate, endDate }) => this.setState({ camapign_created_on_start: startDate, camapign_created_on_end: endDate })}
                                focusedInput={this.state.cc_focusedInput}
                                onFocusChange={cc_focusedInput => this.setState({ cc_focusedInput })}
                                orientation={this.state.orientation}
                                openDirection={this.state.openDirection}
                                disabled={false}
                                isOutsideRange={() => false}
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        loadCampaigns, loadLeadStatuses, listUsers, listGroups, createReport, listQueues
    }, dispatch);
}
